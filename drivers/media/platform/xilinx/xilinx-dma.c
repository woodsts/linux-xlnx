...
static void xvip_dma_buffer_queue(struct vb2_buffer *vb)
{
	...
	if (V4L2_TYPE_IS_MULTIPLANAR(dma->format.type)) {
		...
		dma->sgl.size = (pix_mp->width * dma->fmtinfo->bpl_factor *
				    padding_factor_nume * bpl_nume) /
				    (padding_factor_deno * bpl_deno);
		dma->sgl.icg = pix_mp->plane_fmt[0].bytesperline -
							dma->sgl.size;
		...
		xvip_bpl_scaling_factor(pix->pixelformat, &bpl_nume,
					&bpl_deno);
		dma->xt.frame_size = dma->fmtinfo->num_planes;
		dma->sgl.size = (pix->width * dma->fmtinfo->bpl_factor *
				    padding_factor_nume * bpl_nume) /
				    (padding_factor_deno * bpl_deno);
		dma->sgl.icg = pix->bytesperline - dma->sgl.size;
		dma->xt.numf = pix->height;
		dma->sgl.dst_icg = dma->sgl.size;
	}
